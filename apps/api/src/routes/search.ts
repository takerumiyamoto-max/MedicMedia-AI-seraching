...
import { autoSearchMaterial } from "../services/auto_search";
...
const searchRoutes: FastifyPluginAsync = async (app) => {
  ...
  app.post("/search/material", async (req, reply) => {
    ...
    try {
      ...
      if (!pdf_id) {
        throw new ApiError("VALIDATION_ERROR", "pdf_id is required", 400, { field: "pdf_id" });
      }
      if (!query) {
        throw new ApiError("VALIDATION_ERROR", "query is required", 400, { field: "query" });
      }
      if (!Number.isFinite(topK) || topK < 1 || topK > 20) {
        throw new ApiError("VALIDATION_ERROR", "top_k must be 1..20", 400, {
          field: "top_k",
          value: body.top_k,
        });
      }
  ...
  // ✅ POST /search/material/auto（PR05）
  app.post("/search/material/auto", async (req, reply) => {
    const request_id = crypto.randomUUID();

    try {
      const body = req.body as { pdf_id?: string; top_k?: number };
      ...
      if (!Number.isFinite(topK) || topK < 1 || topK > 50) {
        throw new ApiError("VALIDATION_ERROR", "top_k must be 1..50", 400, {
          field: "top_k",
          value: body.top_k,
        });
      }

      const out = await autoSearchMaterial(uploadDirAbs, env, { pdf_id, top_k: topK });
      ...
      app.log.error({ err, request_id }, "Unhandled error in /search/material/auto");
      return reply.code(500).send({
        ok: false,
        request_id,
        error: { code: "INTERNAL", message: "Internal server error", details: {} },
